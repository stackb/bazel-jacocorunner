...
/**
 * Runner class used to generate code coverage report when using Jacoco offline
 * instrumentation.
 *
 * <p>
 ...
 * <li>JACOCO_METADATA_JAR - specifies jar containing uninstrumented classes to
 * be analyzed.
 * </ul>
 */
public class JacocoCoverageRunner {
  ...
  /**
   * Creates a new coverage runner extracting the classes jars from a wrapper
   * file. Uses
   * javaRunfilesRoot to compute the absolute path of the jars inside the wrapper
   * file.
   */
  ...
  public void create() throws IOException {
    ...
    // Run the structure analyzer on a single class folder or jar file to build up
    // the coverage
    // model. Typically you would create a bundle for each class folder and each jar
    // you want in
    // your report. If you have more than one bundle you may need to add a grouping
    // node to the
    // report. The lcov formatter doesn't seem to care, and we're only using one
    // bundle anyway.
  ...
  void createReport(
    ...
    try (PrintWriter writer = new PrintWriter(newBufferedWriter(reportFile.toPath(), UTF_8, CREATE, APPEND))) {
      final IReportVisitor visitor = formatter.createVisitor(writer, branchDetails);

      // Initialize the report with all of the execution and session information. At
      // this point the
      ...
      // Note the API requires a sourceFileLocator because the HTML and XML formatters
      // display a
      // page of code annotated with coverage information. Having the source files is
      // not actually
      ...
      // Signal end of structure information to allow report to write all information
      // out
      visitor.visitEnd();
    }
  }
  ...
  IBundleCoverage analyzeStructure() throws IOException {
    ...
    final MyCoverageBuilder myCoverageBuilder = new MyCoverageBuilder(coverageBuilder);
    final Analyzer analyzer = new Analyzer(execFileLoader.getExecutionDataStore(), myCoverageBuilder);
    ...
    // TODO(bazel-team): Find out where the name of the bundle can pop out in the
    // report.
    final IBundleCoverage bundle = coverageBuilder.getBundle("isthisevenused");
    return bundle;
  }
   ...
   *
   * <p>
   * The paths are retrieved from a txt file that is found inside each jar
   * containing
   * uninstrumented classes. Each line of the txt file represents a path to be
   * added to the set.
   *
   * <p>
   * This set is needed by {@link JacocoLCOVFormatter} in order to output the
   * correct path for
   * each covered class.
   */
  @VisibleForTesting
  ...
  /**
   * Adds to the given {@link Set} the paths found in a txt file inside the given
   * jar.
   *
   * <p>
   * If a jar contains uninstrumented classes it will also contain a txt file with
   * the paths of
   * each of these classes, called "-paths-for-coverage.txt". This file expects
   * one path per line
   * specified as either:
   *
   * <ul>
  ...
  private static Class<?> getMainClass(boolean insideDeployJar) throws Exception {
    Class<?> mainClass;
    // If we're running inside a deploy jar we have to open the manifest and read
    // the value of
    // "Coverage-Main-Class", set by bazel.
    // Note ClassLoader#getResource() will only return the first result, most likely
    // a manifest
    ...
    // Check JACOCO_MAIN_CLASS after making sure we're not running inside a deploy
    // jar, otherwise
  ...
  private static String getUniquePath(String pathTemplate, String suffix) throws IOException {
    // If pathTemplate is null, we're likely executing from a deploy jar and the
    // test framework
    // did not properly set the environment for coverage reporting. This alone is
    // not a reason for
    // throwing an exception, we're going to run anyway and write the coverage data
    // to a temporary,
    // throw-away file.
    if (pathTemplate == null) {
      return File.createTempFile("coverage", suffix).getPath();
    } else {
      // bazel sets the path template to a file with the .dat extension. lcov_merger
      // matches all
      // files having '.dat' in their name, so instead of appending we change the
      // extension.
  ...
  /**
   * Returns an immutable list containing all the file paths found in mainFile. It
   * uses the
  ...
  private static URL[] getUrls(ClassLoader classLoader, boolean wasWrappedJar) {
    ...
    // If the classpath was too long then a temporary top-level jar is created
    // containing nothing
    // but a manifest with
    // the original classpath. Those are the URLs we are looking for.
    if (wasWrappedJar && urls != null && urls.length == 1) {
      try {
        String jarClassPath = new JarInputStream(urls[0].openStream())
            .getManifest()
            .getMainAttributes()
            .getValue("Class-Path");
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (urls != null) {
      ...
      for (int i = 0; i < urls.length; i++) {
        ...
        if (file.endsWith(".jar")) {
          ...
          // Collecting these in the shutdown hook is too expensive (we only have a 5s
          // budget).
    ...
    // (shutdown hooks are
    // not guaranteed any particular order). Note that also by default, Jacoco
    // appends coverage
    // data, which can have surprising results if running tests locally or somehow
    // encountering
    // the previous .exec file.
    System.setProperty("jacoco-agent.output", "none");

    // We have no use for this sessionId property, but leaving it blank results in a
    // DNS lookup
    // at runtime. A minor annoyance: the documentation insists the property name is
    // "sessionId",
    // however on closer inspection of the source code, it turns out to be
    // "sessionid"...
    System.setProperty("jacoco-agent.sessionid", "default");

    // A JVM shutdown hook has a fixed amount of time (OS-dependent) before it is
    // terminated.
    // For our purpose, it's more than enough to scan through the instrumented jar
    // and match up
    // the bytecode with the coverage data. It wouldn't be enough for scanning the
    // entire classpath,
    ...
    Runtime.getRuntime()
        .addShutdownHook(
            new Thread() {
              ...
              public void run() {
                try {
                  // If the test spawns multiple JVMs, they will race to write to the same files.
                  // We
                  ...
                  // about merging them.
                  String coverageReportDat = getUniquePath(coverageReportBase, ".dat");
                  String coverageData = getUniquePath(coverageReportBase, ".exec");

                  // Get a handle on the Jacoco Agent and write out the coverage data. Other
                  // options
    ...
    // It can happen that multiple deploy jars are on the classpath. In that case we
    // are running
    // from a regular java binary where all the environment (e.g. JACOCO_MAIN_CLASS)
    // is set
    // accordingly.
    ...
    // Another option would be to run the tests in a separate JVM, let Jacoco dump
    // out the coverage
    // data, wait for the subprocess to finish and then generate the lcov report.
    // The only benefit
    // of doing this is not being constrained by the hard 5s limit of the shutdown
    // hook. Setting up
    // the subprocess to match all JVM flags, runtime classpath, bootclasspath, etc
    // is doable.
    // We'd share the same limitation if the system under test uses shutdown hooks
    // internally, as
  ...
  static class MyCoverageBuilder implements ICoverageVisitor {
    ...
    MyCoverageBuilder(CoverageBuilder delegate) {
