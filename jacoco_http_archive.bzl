...
def _jacoco_http_archive_impl(ctx):
    ...
    mvn = ctx.path(ctx.attr.mvn)
    java_home = ctx.path(ctx.attr.java_home).dirname
    ...
    ctx.file("repo/BUILD.bazel", "")

    result = ctx.execute([
        "%s" % mvn,
        "-Dmaven.repo.local=%s" % ctx.path("repo"),
        "-Dmaven.compiler.source=11",
        "-Dmaven.compiler.target=11",
        "-pl=!org.jacoco.core.test.validation.groovy,!org.jacoco.examples.test,!org.jacoco.doc",  # getting weird error
        "install",
        "-DskipTests=true",
    ], quiet = False)
    if result.return_code != 0:
    ...
    ctx.extract("repo/org/jacoco/jacoco/{version}/jacoco-{version}.zip".format(version = ctx.attr.version))
...
java_import(
  name = 'agent',
  jars = glob(["org.jacoco.agent-*.jar"]),
)

java_import(
  name = 'core',
  jars = glob(["org.jacoco.core-*.jar"]),
)

java_import(
  name = 'report',
  jars = glob(["org.jacoco.report-*.jar"]),
)

java_import(
  name = 'ant',
  jars = glob(["org.jacoco.ant-*.jar"]),
)
...
""")
    ...
    return update_attrs(ctx.attr, _jacoco_http_archive_attrs.keys(), sha256_override)
...
`"tar.xz"`, `"txz"`, `"tar.zst"`, `"tzst"`, `tar.bz2`, `"ar"`, or `"deb"`.""",
    ...
    "version": attr.string(
        default = "0.8.6",
        doc =
            "the jacoco version string",
    ),
    "mvn": attr.label(
        default = "@apache_maven//:bin/mvn",
        executable = True,
        allow_files = True,
        cfg = "exec",
        doc =
            "The path to the bin/mvn script.",
    ),
    "java_home": attr.label(
        # default = "@remotejdk11_macos//:BUILD.bazel",
        default = "@remotejdk17_macos//:BUILD.bazel",
        executable = True,
        allow_files = True,
        cfg = "exec",
        doc =
            "A file at the root of a jdk used to locate JAVA_HOME for mvn",
    ),
}

jacoco_http_archive = repository_rule(
    implementation = _jacoco_http_archive_impl,
    attrs = _jacoco_http_archive_attrs,
    doc =
